...
time_point<high_resolution_clock> brisKetFoundTime = high_resolution_clock::now(); // para imprimirlo al final, no sacar pls

// Tiempo sin mejorar el óptimo antes de detener la búsqueda
static constexpr double TIMEOUT_NO_OPTIMUM_SEC = 300.0;
...
// El perrowacho que vigila tiempo sin nuevo óptimo y activa timeoutFired - Ayuda de IA
void watchdogThread() {
...
// Hecho con IA, adaptado para que funcione en Windows y Unix
void keyboardThread() {
#ifdef _WIN32
    while (searchActive.load(memory_order_acquire)) {
        if (_kbhit()) {
...
void initDomains() {
    // Dominio completo: todos los enteros en [E_k, L_k] para cada avión k
    domains.resize(D);
    for (int k = 0; k < D; k++) {
        int lo = planes[k].E, hi = planes[k].L;
        domains[k].resize(hi - lo + 1);
        iota(domains[k].begin(), domains[k].end(), lo);
    }
}
                         ...
                         int trailLevel)
{
    for (int future = 0; future < D; future++) {
        ...
        // (dominio completo: no se insertan puntos adicionales)
...
int main(int argc, char* argv[]) {
    ...
    for (int arg = 2; arg < argc; arg++) {
        ...
        cout << "Dominio completo por avion:\n";
        for (int i = 0; i < D; i++)
            cout << "  Avion " << setw(2) << i
                 << ": " << setw(5) << domains[i].size() << " valores"
                 << "  [" << domains[i].front()
                 << " .. " << domains[i].back() << "]\n";
        cout << "\n";
