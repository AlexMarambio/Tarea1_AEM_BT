...
// Heuristicas: Smallest Window First + Degree, Valor mas cercano a P_k
// Alex Marambio
// ============================================================
...
int selectVariable(const vector<bool>& assigned) {
    ...
    for (int i = 0; i < D; i++) {
        ...
        int window = planes[i].L - planes[i].E;   // Smallest Window First
        int degree = 0;
        for (int j = 0; j < D; j++) {
        ...
        if (window < minWindow) {
            ...
            bestDegree = degree;
            best       = i;
        } else if (window == minWindow && degree > bestDegree) {
            bestDegree = degree;
            best       = i;
        }
    }
...
void printSolution() {
    ...
    cout << "\n=== MFC  (Smallest Window First + Degree) ===\n";
